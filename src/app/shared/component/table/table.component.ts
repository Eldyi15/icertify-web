...
export class TableComponent implements OnInit {
  ...
  constructor(private api: ApiService, public util: UtilService, private dialog: MatDialog, private _bottomSheet: MatBottomSheet,) { }
  ...
  onRowClick(data: any, index: number) {
    ...
    }).afterDismissed().subscribe((res: any) => {
      console.log(res)
      if (res) {

        if (res === "Delete") { data.status = "Deleted" }
        if (res === "Suspend") { data.status = "Suspended" }
        if (res === "Activate") { data.status = "Active" }
        console.log(data)
        switch (res) {
          case 'View':
          case 'Update':
